...
Cross-Entropy, Dice, Focal, and combined losses, we find that (i) ImageNet
pre-training contributes a $+7.1$ mIoU gain that dwarfs the $\le 1.5$ gain
from switching between decoders; (ii) a combined CE~+~Dice loss increases the
Barren-class IoU from 0.571 to 0.676 (a $+10.5$ absolute-point improvement)
while also raising mean IoU by $+1.8$; and (iii) a deeper ResNet101 encoder
underperforms ResNet50 by $-0.08$ mIoU despite using $71\%$ more parameters and
being $\sim11\%$ slower at inference. The best configuration
(DeepLabV3+-R50~+~Combined) reaches 0.733 test mIoU on full-resolution
reconstructions, exceeding published DeepGlobe baselines.
...
\begin{enumerate}
\item \textbf{Transfer learning contributes far more than architecture.}
      Replacing Vanilla U-Net with a ResNet34-backboned U-Net (both
      randomly initialized) improves val mIoU by $+0.030$, a
      purely-architectural contribution. Switching from random
      initialization to ImageNet pre-trained encoder adds a further
      $+0.041$. Subsequent architectural changes from ResNet34 $\rightarrow$
      DeepLabV3+-R50 $\rightarrow$ Attention U-Net yield only $+0.010$
      combined. The 7.1-point ``pre-training jump'' dominates the $\le 1.5$
      architectural delta that the field has historically emphasized.
\item \textbf{ResNet101 does not beat ResNet50.} Despite having 71\% more
      parameters (45.67M vs.~26.68M), DeepLabV3+-R101 reaches $0.7124$
      val mIoU versus R50's $0.7201$. This contradicts the proposal's
      pre-registered prediction that R101 would be $\sim$1--2 points higher,
      and we attribute it to two factors: (i) the training set of
      $\sim$5000 tiles is small relative to R101's capacity, inducing
      overfitting; (ii) the bs=4 constraint (R101 does not fit at bs=8)
      increases gradient noise in the AMP optimizer relative to R50's bs=8.
      The practical consequence is that R101 is slower ($+53$ ms/image at
      inference) and more memory-hungry without accuracy benefit.
\item \textbf{Attention U-Net wins overall}, but only by $+0.006$ val mIoU
      over DeepLabV3+-R50. Given that R50 trains 2.3$\times$ faster, it is
      arguably the better choice when many loss-function ablations are
      planned---this is the rationale for using R50 in Phase 5.
\end{enumerate}
...
Rangeland IoU stays in the 0.3--0.4 range for every architecture; it is the
class most commonly confused with Forest.}
\label{fig:arch-bars}
\end{figure}
...
$0.5\cdot\text{CE}+0.5\cdot\text{Dice}$.}\label{tab:loss}
\begin{tabular}{l c c c c c}
\toprule
Loss        & Val mIoU & Test mIoU & Barren IoU (test) & Water IoU (test) & Rangeland IoU (test) \\
\midrule
...
winner). Both models share the failure mode of confusing Rangeland with
Forest and Agriculture---these classes are spectrally and texturally
similar in satellite imagery.
...
predicted as Forest, (ii) urban shadows classified as Water, and (iii)
...
a validation mIoU within 0.006 of the best (Attention U-Net) while training
2.3$\times$ faster and using comparable GPU memory. ResNet101 is strictly
dominated: more parameters, more memory, slower inference, \emph{lower}
accuracy. Vanilla U-Net has the largest parameter count (31.0M) yet the
lowest accuracy, suggesting that for this task the encoder's inductive
biases and ImageNet pre-training matter more than raw capacity.
...
\begin{enumerate}
\item \textbf{Pre-training is the single largest lever} (+7.1 mIoU vs.~any
      architectural change, which contributes $\le 1.5$). Practitioners
      facing resource constraints should first ensure a pre-trained
      backbone before evaluating architectural variants.
\item \textbf{Combined CE~+~Dice loss is the most effective defense
      against class imbalance} in our setting, improving minority
      (Barren) IoU by 10.5 points over the weakest baseline and 2.0
      points over CE with identical architecture.
\item \textbf{Deeper encoders do not automatically help}: DeepLabV3+-R101
      underperforms R50 by $-0.08$ mIoU while being 53 ms/image slower and
      requiring bs=4 instead of bs=8. At the training-set scale of
      DeepGlobe, R101's extra capacity appears to overfit.
\end{enumerate}
...
test set, exceeding published baselines for this dataset.
