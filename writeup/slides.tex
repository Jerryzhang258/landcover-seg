...
\begin{frame}{Experimental grid}
  ...
  \begin{itemize}
    \setlength{\itemsep}{0.15em}
    \item Vanilla U-Net (scratch)
    \item U-Net + ResNet34 (scratch)
    \item U-Net + ResNet34 (ImageNet)
    \item DeepLabV3+ + ResNet50 (ImageNet)
    \item DeepLabV3+ + ResNet101 (ImageNet)
    \item Attention U-Net + R34 + SCSE (ImageNet)
  \end{itemize}

  \column{0.48\textwidth}
  \textbf{Phase 5} -- loss, fixed architecture, 4 runs:
  \begin{itemize}
    \setlength{\itemsep}{0.15em}
  ...
  \vspace{0.7em}
  \emph{Architecture choice for Phase 5:} DeepLabV3+-R50 --- best efficient
  performer in Phase 4 (2.3$\times$ faster training than Attention U-Net,
  within $0.006$ val mIoU).
...
\begin{frame}{Confusion matrices}
  ...
  \vspace{0.2em}
  \footnotesize
  Shared failure mode: Rangeland misclassified as Agriculture (row
  fraction $\sim 0.21$--$0.23$)---the two classes share overlapping
  spectral and textural signatures in RGB imagery.
\end{frame}
...
\begin{frame}{Failure cases (DeepLabV3+-R50 + Combined)}
  ...
  \vspace{0.2em}
  \footnotesize
  Errors cluster at semantic boundaries: Rangeland $\to$ Agriculture,
...
\begin{frame}{Summary of findings}
  ...
  \begin{enumerate}
    ...
    \item \textbf{Pre-training dominates.} $+4.1$ mIoU (architecture
          fixed) vs.\ $\le +1.0$ for any single decoder swap.
          \textit{$\Rightarrow$ prioritize pre-training over architecture.}
    \item \textbf{Loss rebalancing complements architecture.}
          Combined CE + Dice on DeepLabV3+-R50 adds $+3.7$ pp test mIoU
          and $+2.0$ pp Barren IoU; stacked with the architecture
          upgrade, Barren reaches $+10.5$ pp.
          \textit{$\Rightarrow$ rebalance the loss for long-tailed classes.}
    \item \textbf{Depth is not Pareto-dominant.} R101 vs.\ R50:
          val $-0.008$, test $+0.022$ (within single-seed variance),
          at $71\%$ more parameters and $2.4\times$ training cost.
  ...
  \begin{block}{Best model}
    \footnotesize
    \textbf{DeepLabV3+-R50 + Combined (CE + Dice) loss} --- \textbf{0.7332
    test mIoU} on full-resolution reconstructions (120 test images at
    $2448 \times 2448$, 7 classes).
  \end{block}
\end{frame}
